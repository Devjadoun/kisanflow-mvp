...
import { Wheat, User, KeyRound, ShieldCheck, CheckCircle2, ArrowRight } from 'lucide-react';
...
export const LoginPage = () => {
  ...
  return (
    <div className="min-h-[80vh] flex flex-col justify-center py-10 px-4 sm:px-6 lg:px-8 max-w-xl mx-auto w-full">
      <div className="mb-4">
        <SimulationNotice compact message="Prototype Login: Click any Quick Role button below for instant 1-click access." />
      </div>
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-200 text-left">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-black text-slate-900">Sign in to KisanFlow</h2>
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase mb-1">
              {selectedTab === 'farmer' ? 'Mobile Number / Aadhaar' : selectedTab === 'operator' ? 'Operator ID' : 'Govt Email / Staff ID'}
            </label>
            <input
              type="text"
              value={phone}
              onChange={(e) => setPhone(e.target.value)}
              className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 focus:outline-hidden focus:ring-2 focus:ring-emerald-600 text-sm font-medium"
          ...
          <div>
            ...
            />
            {selectedTab === 'farmer' && (
              <p className="text-[11px] text-slate-400 mt-1">Demo OTP auto-filled: 123456</p>
            )}
          </div>
          ...
          >
            <span>Proceed to {selectedTab.charAt(0).toUpperCase() + selectedTab.slice(1)} Dashboard</span>
        ...
        <div className="mt-6 pt-6 border-t border-slate-200">
          ...
          <div className="space-y-2">
            ...
            >
              <span>🌾 Login as <strong>Ramesh Kumar</strong> (Farmer)</span>
              <span className="text-[10px] bg-emerald-200 px-1.5 py-0.5 rounded text-emerald-800">Token A027</span>
            </button>
            <button
            ...
            >
              <span>🏢 Login as <strong>Dadri Mandi Operator</strong></span>
              <span className="text-[10px] bg-blue-200 px-1.5 py-0.5 rounded text-blue-800">Weighbridge</span>
            </button>
            <button
            ...
            >
              <span>🏛️ Login as <strong>State Admin / Official</strong></span>
              <span className="text-[10px] bg-purple-200 px-1.5 py-0.5 rounded text-purple-800">Analytics</span>
            </button>
          </div>
        </div>
      </div>
